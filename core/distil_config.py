...
class DistilConfig(TrainingArguments):
    ...
    full_logit_distillation: bool = field(
        default=True,
        metadata={
            "help": "If True, compute KL divergence over the full vocabulary distribution at each token position "
                   "(analytic per-token estimator). If False, use only the selected token log-probs."
        },
    )
